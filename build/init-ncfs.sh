...
setup_ngrok () {
    ...
    if [[ -n "${CLOUDFLARE_AUTH_EMAIL}" || -n "${CLOUDFLARE_API_KEY}" || -n "${CLOUDFLARE_ZONE_ID}" || -n "${CLOUDFLARE_CNAME_RECORD}" || -n "${CLOUDFLARE_SRV_RECORD}" ]]; then
        setup_cloudflare
        echo "[$(date +"%T") SUC]: Done! Your server is now available at ${CLOUDFLARE_SRV_RECORD}"
    else
...
setup_cloudflare () {
    ...
    cname_record=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/${CLOUDFLARE_ZONE_ID}/dns_records?type=CNAME&name=${CLOUDFLARE_CNAME_RECORD}" \
    ...
    if [[ $cname_record == *"\"count\":0"* ]]; then
        echo "[$(date +"%T") ERR]: CNAME record does not exist in Cloudflare. You have to create it manually. Create a CNAME record in your Cloudflare dashboard and set the name to ${CLOUDFLARE_CNAME_RECORD} (you can put example.com to content for now)"
        exit 1
    fi
    ...
    srv_record=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/${CLOUDFLARE_ZONE_ID}/dns_records?type=SRV&name=_minecraft._tcp.${CLOUDFLARE_SRV_RECORD}" \
                        -H "X-Auth-Email: ${CLOUDFLARE_AUTH_EMAIL}" \
                        -H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
                        -H "Content-Type: application/json")
    ...
    if [[ $srv_record == *"\"count\":0"* ]]; then
        echo "[$(date +"%T") ERR]: SRV record does not exist in Cloudflare. You have to create it manually. Create a SRV record in your Cloudflare dashboard and set the name to ${CLOUDFLARE_SRV_RECORD} (you can put ${CLOUDFLARE_CNAME_RECORD} to content for now)"
        exit 1
    fi
    ...
    update=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/${CLOUDFLARE_ZONE_ID}/dns_records/$cname_record_id" \
                         ...
                         -H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
                         -H "Content-Type: application/json" \
                         --data "{\"type\":\"CNAME\",\"name\":\"${CLOUDFLARE_CNAME_RECORD}\",\"content\":\"$ngrok_host\"}")
    ...
    case "$update" in
        *"\"success\":false"*)
        ...
        ;;
        *)
            echo "[$(date +"%T") INFO]: CNAME record updated in Cloudflare. $ngrok_host - ${CLOUDFLARE_CNAME_RECORD}"
        ;;
    esac
    ...
    update=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/${CLOUDFLARE_ZONE_ID}/dns_records/$srv_record_id" \
                         ...
                         -H "X-Auth-Key: ${CLOUDFLARE_API_KEY}" \
                         -H "Content-Type: application/json" \
                         --data "{\"type\":\"SRV\",\"name\":\"_minecraft._tcp.${CLOUDFLARE_SRV_RECORD}\",\"data\": {\"name\":\"${CLOUDFLARE_SRV_RECORD}\",\"port\":$ngrok_port,\"proto\":\"_tcp\",\"service\":\"_minecraft\",\"target\":\"${CLOUDFLARE_CNAME_RECORD}\"}}")
    ...
    case "$update" in
        *"\"success\":false"*)
        ...
        ;;
        *)
            echo "[$(date +"%T") INFO]: SRV record updated in Cloudflare. $ngrok_host - _minecraft._tcp.${CLOUDFLARE_SRV_RECORD}"
        ;;
    esac
}
